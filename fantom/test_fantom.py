...
EVAL_DIR_PATH = os.path.join(PROJECT_HOME, 'scores', 'fantom_ssz32')
RANDOM_SEED = 99
random.seed(RANDOM_SEED)
ANSWER_PROMPT_WITH_THEREFORE = "Therefore, the short answer without explanation is:"
ANSWER_PROMPT_WITH_THEREFORE_MC = "Therefore, the chosen answer from the above options is:"
TRACE_HEADER = "Please use this rewritten context when answering the question above:"
...
class FantomEvalAgent():
    def __init__(self, args):
        self.args = args
        self.prompt_header = "This is a theory-of-mind test. Please answer the question regarding facts or beliefs, based on the following in-person conversation between individuals who have just met. Only output the answer without explanation.\n\n"
        model_name_suffix = self.args.model.split("/")[-1]
        self.output_filename_suffix = '_{}_input_{}_tracing-{}_cot-{}_runid-{}.json'.format(self.args.conversation_input_type, model_name_suffix, self.args.use_tracing, self.args.use_cot, self.args.run_id)
        self.load_fantom()
        self.setup_fantom()

        if self.args.use_tracing:
            self.model = load_tracer_model(args)
        else:
            self.model = load_model(self.args.model, **vars(args))
        if "/" in args.model:
            self.model_name = self.args.model.split("/")[-1]
        else:
    ...
    def load_fantom(self):
        file = os.path.join(DATA_DIR_PATH, "fantom", "fantom_v1.2.json")
        self.fantom_df = pd.read_json(file)
    ...
    def map_binary_answer_to_int(self, model_response):
        ...
        """
        model_answer = model_response.lower().strip("'").strip('"').strip("*")
        if " yes," in model_answer or " yes " in model_answer or model_answer.startswith("yes") or " yes." in model_answer or " knows " in model_answer or "{yes}" in model_answer or model_answer.lower().startswith("true"):
            return 1
        elif " no," in model_answer or " no " in model_answer or model_answer.startswith("no") or " no." in model_answer or " does not know " in model_answer or " doesn't know " in model_answer or "{no}" in model_answer or model_answer.lower().startswith("false"):
            return 0
        else:
            return -1
    ...
    def evaluate_response(self, qas, predictions):
        ...
        predictions = predictions[:len(qas)]
    ...
    def setup_fantom(self):
        ...
        # self.fantom_df_to_run = self.fantom_df.sample(n=100, random_state=RANDOM_SEED)
        # self.fantom_df_to_run = self.fantom_df.iloc[:30]
        self.fantom_df_to_run = self.fantom_df.iloc[:self.args.n_sets]
    ...
    def extract_question(self, input_prompt):
        if "Target: " in input_prompt:
            question = "Target: " + input_prompt.split("\nTarget:")[-1].strip()
        elif "Information: " in input_prompt:
            question = "Information: " + input_prompt.split("\nInformation:")[-1].strip()
        elif "Question: " in input_prompt:
            question = "Question: " + input_prompt.split("\nQuestion:")[-1].strip()
    ...
    def run_batch_inference(self):
        ...
        for batch_idx, batch in enumerate(tqdm(loader)):
            ...
            if "qwen" in self.args.model.lower():
                responses, logprobs = self.model.batch_interact(batch)
            else:
    ...
    def run_inference(self):
        ...
        if self.args.existing_traces is not None:
            existing_traces = os.path.join(os.path.dirname(__file__), self.args.existing_traces) 
            df = pd.read_json(existing_traces, lines=True)
        ...
        for idx, input_prompt in enumerate(tqdm(target_data)):
            ...
            question = self.extract_question(input_prompt)
            if self.args.use_tracing and self.args.existing_traces is None:
                if "Answer:" in input_prompt:
                    tracing_input = input_prompt.split("Answer:")[0].strip()
                    generated_thought = self.model.trace(tracing_input).strip()
                elif "Choose an answer from above:" in input_prompt:
                    tracing_input = input_prompt.split("Choose an answer from above:")[0].strip()
                    generated_thought = self.model.trace(tracing_input).strip()
                else:
                    print(">>> Input prompt does not contain 'Answer:' or 'Choose an answer from above:'")
                if generated_thought != "":
                    input_prompt = input_prompt + f"\n\n{generated_thought}\n\n{ANSWER_PROMPT_WITH_THEREFORE}"

            if self.args.existing_traces is not None:
                # when you use the existing traces, the input prompt includes the trace.
                input_prompt = df.iloc[idx]['input_prompt']

            print(Panel(input_prompt, title="Input prompt"))
            if self.args.use_cot:
                input_prompt = input_prompt.strip().removesuffix("Answer:")
                input_prompt = input_prompt.strip().removesuffix(ANSWER_PROMPT_WITH_THEREFORE)
                cot_input_prompt = input_prompt + "Let's think step by step."
                cot_response = self.model.interact(cot_input_prompt, temperature=0.7, max_tokens=512)
                cot_response = self.parse_response(cot_response)
                print(Panel(cot_response, title="CoT reasoning", style="blue"))
                input_prompt = cot_input_prompt + " " + cot_response + "\n\n" + ANSWER_PROMPT_WITH_THEREFORE

            input_prompt = input_prompt.replace("\nAnswer", " Do not add extra comments.\nAnswer")
            _response = self.model.interact(input_prompt, temperature=0, max_tokens=200)

            if self.args.model == "deepseek-ai/DeepSeek-R1":
                thoughts = _response.split("</think>")[0].strip()
                prediction = _response.split("</think>")[-1].strip()
            elif self.args.model.startswith("o1") or self.args.model.startswith("o3"):
                prediction = _response['response']
                n_reasoning_tokens = _response['n_reasoning_tokens']
            else:
                prediction = _response

            response = self.parse_response(prediction)
            model_responses.append(response)

            print(Panel(f"{_response}", title="Model response", style="green"))
            print()
            print()
            ...
            with open(response_filename_path, 'a') as f:
                if self.args.model == 'deepseek-ai/DeepSeek-R1':
                    json.dump({'index': idx, 'input_prompt': input_prompt, 'response': response, 'thoughts': thoughts}, f)
                elif self.args.model.startswith("o1") or self.args.model.startswith("o3"):
                    json.dump({'index': idx, 'input_prompt': input_prompt, 'response': response, 'n_reasoning_tokens': n_reasoning_tokens}, f) 
                else:
    ...
    def run(self):
        ...
        wandb.init(
            ...
            tags=[self.args.model, self.args.run_id, f"tracing-{self.args.use_tracing}", f"cot-{self.args.use_cot}", self.args.tracer_type, f"n-sets-{self.args.n_sets}", f"fantom-{self.args.conversation_input_type}", f"aggregation-{self.args.aggregation_target}"],
            save_code=True
        )
        wandb.run.save("../*tracer*.py", base_path="../", policy="now")

        if args.existing_response_file_name is None:
            if self.args.use_tracing or self.args.model.startswith("text-") or self.args.batch_size == 1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n-sets', type=int, default=64, help='number of sets to evaluate')
